...
ZTEST(slot, test_periodic_on_time_early_late)
{
	...
	/* Late 20 s (busy work queue): same slot, next one pulled in. */
	zassert_equal(app_slot_next(&s, I, 1140, false, 1140, true, &slot), 40);
...
ZTEST(slot, test_rtc_steps)
{
	...
	/* Clock set forward by 1 h 17 s: grid phase kept. */
	uint32_t now = UNX + 3600 + 17;

	zassert_equal(app_slot_next(&s, I, now, true, 0, true, &slot), I - 17);
	zassert_equal(slot, UNX + 3600);

	/* Clock set back before the anchor (negative distance): still on the grid. */
	now = UNX - 1000;
	uint32_t d = app_slot_next(&s, I, now, true, 0, true, &slot);

	zassert_equal(slot, app_slot_round(UNX, I, now));
	zassert_equal((int32_t)(slot - UNX) % (int32_t)I, 0, "off grid");
	zassert_true(d >= I / 2 && d <= I + I / 2, "delay %u", d);
}
