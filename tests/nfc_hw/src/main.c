 ...
 * accepts them (ret==0), which lets handle_encrypted_cmd() run to completion:
 * #415 removed the implicit claim-window close it used to do there. */
...
/* #415: a provisioned device is ACTIVE by default (no lazy arm). Provision the
 * token, init, and lay the resting NDEF so the clm record is on the tag. */
static void setup_claim_active(void)
{
	memset(g_app_config.claim_token, 0xAB, sizeof(g_app_config.claim_token));
	zassert_equal(app_nfc_init(), 0, "app_nfc_init failed");
	zassert_equal(app_nfc_check(), 0, "resting poll (empty tag) failed");
	zassert_equal(app_nfc_claim_state_get(), APP_NFC_CLAIM_ACTIVE,
		      "test precondition: claim window active");
}

/* #415 D10: a valid secret_key command no longer closes the claim window (the
 * #308 implicit close is gone) — it stays ACTIVE until an explicit claim_done. */
ZTEST(nfc_hw, test_secret_key_command_does_not_close_claim_window)
{
	setup_claim_active();
	 ...
	 * leftover bytes from setup_claim_active()'s earlier resting-NDEF write don't
	...
	zassert_equal(app_nfc_claim_state_get(), APP_NFC_CLAIM_ACTIVE,
		      "#415: a decrypted command must NOT close the claim window");
}

/* A vendor_token command likewise leaves the window ACTIVE (it never had the
 * implicit close, #316; #415 makes that true for every transport). */
ZTEST(nfc_hw, test_vendor_token_command_does_not_close_claim_window)
{
	setup_claim_active();
	...
	zassert_equal(app_nfc_claim_state_get(), APP_NFC_CLAIM_ACTIVE,
		      "a vendor_token decrypt must leave the claim window active");
}
