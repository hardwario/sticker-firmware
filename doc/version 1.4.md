...
- **Idle timeout** — `CONFIG_APP_DEBUG_AUTOSUSPEND_S` (default **7200 s** = 2 h). After this long with no **RTT/shell interaction**, the device enters deep sleep. Any shell input resets the timer. Set to `0` to disable. Debug-only (no effect on the Release build, which already sleeps via PM).
- **Paused while debugging** — auto-suspend is skipped whenever a debugger is attached (CoreDebug `C_DEBUGEN`): during an active SWD session the MCU would only wake from Shutdown via DBGMCU and boot-loop, so it stays awake; the idle window starts fresh once the probe detaches. A forgotten battery unit (no debugger) is still protected.
