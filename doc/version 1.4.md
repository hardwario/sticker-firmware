...
- **Audit small-fixes batch (#267)** — a set of small, independent correctness / power / robustness fixes from the v1.4.0 audit tracker:
  - **Report jitter fired reports early** — the ±jitter on `interval_report` used a **signed** modulo (`(int32_t)sys_rand32_get() % …`), negative ~half the time, so reports fired *before* the interval and the real uplink rate ran above what's configured (extra airtime + battery). Now an unsigned `0..interval/10` jitter (always ≥ 0, spreads later only), plus a guard for `interval_report < 10` (a latent modulo-by-zero).
