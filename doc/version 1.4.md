...
> **#348 — `alarm-notif-time` and the illuminance-only `alarm-light-confirm-delay` were removed.** Both were global timings (one red-LED hold time for every pulse/edge alarm on the device; one confirm-delay for illuminance only). They are replaced by the per-rule `dwell` field already present in every alarm slot (formerly named `hst`) — see the per-kind `dwell` description under **"Dynamic alarm rules"** below. This is a breaking change to the `alarms` config group (proto ids 2 and 19 are now `reserved`, never reused) — acceptable because no device is provisioned with alarm rules in the field yet (all rules default to `dwell = 0`, which reproduces the pre-#348 immediate-fire behavior for every kind except THRESHOLD, where it also drops the old hysteresis band — see below).
...
> **#338 — buzzer HW variant.** A HW variant exists where a CMI-9605IC-0380T self-driven magnetic buzzer is wired directly across the *same* PB4/PA11 pins as the PIR sensor, instead of a real PIR. A new `cap-buzzer` capability (bool, `sensors` proto group) enables it. Because it's on the same pins, the arbitration above becomes 3-way: **PIR wins over both `cap-buzzer` and the digital inputs**; if PIR is off, **`cap-buzzer` wins over the digital inputs**. `app_buzzer.c` exposes only a fixed, non-blocking control surface — no caller can construct a custom on/off sequence: `app_buzzer_on(ms)` (bounded on-pulse, local/`ats` only) and `app_buzzer_play()`/`app_buzzer_play_repeating()`, which trigger a melody **by numeric id** (single beep / double beep / fast 5x beep, analogous to the LED severity scale in §16) rather than by name — `app_buzzer.c`'s internal melody table is the *only* place that decides which ids exist, so adding a new melody later never touches the wire format. Playback runs on a dedicated thread so triggering a melody never blocks the caller; `app_buzzer_set(false)` (`ats buzzer off`) stops immediately, aborting a beep/melody that's mid-playback rather than waiting it out. Reachable locally as `ats buzzer off|beep <ms>|play <info|warning|alarm> [repeat_s]` (gated on `cap-buzzer`), and remotely over **NFC or LoRaWAN** as the new `buzzer_play` Command (proto id 28): `kind` (0 = **stop** — silences immediately, aborting a mid-playback beep/melody, and cancels a pending repeat cycle; also the default for an omitted field, so an empty `buzzer_play` is a stop; 1 = info; 2 = warning; 3 = alarm; 4-15 reserved; any value ≥ 16 collapses to 0) and `repeat_s` (0 = play once; 1-999 = replay that many seconds after the melody finishes, indefinitely, until stopped — remotely with `kind: 0`, locally with `ats buzzer off`). A reserved `kind` (4-15) is rejected with `BAD_REQUEST`, detected by `app_buzzer_play_repeating()` itself, not the command dispatcher. **The Ack is fire-and-forget**: it confirms the request was accepted, not that it audibly played — a newer request supersedes an older one that hasn't started yet, and a request stuck behind a long playback for more than ~2 s is dropped as stale. Out of scope: any PWM/tone generation (the part is DC self-oscillating only). Alarm-driven playback landed in Phase 2 (#397, v1.5.0) via `app_alarm_poll()`'s aggregate "any alarm active" result — not `app_alarm.c`'s per-source `app_alarm_event()` callback, which only ever sees raw GPIO edges (hall/input/PIR/accel) and would miss threshold/rate/no-data/low-battery alarms entirely. See `doc/version 1.5.md` §1.
...
- **threshold** (analog) — plain `lo`/`hi` band, no hysteresis; alarm when the value leaves `[lo, hi]`. `dwell` (seconds) is a **dwell**: the value must stay continuously outside the band for `dwell` seconds before the alarm activates (`0` = immediate, the pre-#348 behavior minus hysteresis). **Deactivation is always immediate** on returning inside `[lo, hi]` — dwell only guards the activating edge against a transient spike/dip; there is no reason to also delay clearing an alarm once the value has recovered.
- **state** (digital: tilt + discrete inputs) — `from_state`/`to_state`. **`from != to` = edge** (fires once on that transition: `0→1` rising, `1→0` falling); **`from == to` = level** (active while the line equals `to`: `1→1` active-high, `0→0` active-low). `dwell` (seconds) here is a **confirm + hold**: for an edge, the raw transition must persist at `to_state` for `dwell` seconds before it fires (debounce), and firing then holds the alarm active — and blocks a new transition from re-arming — for that same `dwell` seconds; for a level, `dwell` gates activation the same way (dwell before activating), but deactivation is immediate once the state leaves `to_state` (same asymmetry as threshold). PIR and accel are *momentary* sources (they only pulse, nothing to debounce), so any state rule there **fires immediately** on each pulse and instead uses `dwell` purely as the **post-fire hold/re-arm window** (a burst of pulses within that window is suppressed).
- **count / rate** (counters: hall, input) — `hi` = maximum events allowed per report interval. The increase is assessed once per **`interval_report` window** (a tumbling window that re-baselines each report), not on every internal poll, so the rate is counted consistently regardless of the sample cadence. `dwell` (seconds) is the **hold/re-arm window** after firing, the same role as a momentary source.

`dwell = 0` means "no timing" throughout: immediate threshold activation (no dwell), immediate state edge/level firing with no confirm and no re-arm block, immediate momentary/count re-arm. `dwell` is range-checked to **0–3600 s** on write, for every kind.
...
| 13–16 | dwell | float32 — **dwell/hold in seconds** (#348; all kinds, see above; field renamed from `hst`). Same offset and size as the pre-#348 threshold-hysteresis field it replaces — no wire-format change, only the FW-side interpretation of the value changed. |
...
Shell `<args>` by kind: threshold `<lo> <hi> [dwell]` · state `<from> <to> [dwell]` · count `<N> [dwell]`. `dwell` defaults to `0` (immediate) when omitted, for every kind.
...
alarm clear all
alarm list                               # [i] <source> <quantity> … dwell=<s>  en=<0|1>  active=<0|1>
```
...
`encodeDownlink` returns `{ bytes, fPort: 85, warnings, errors }`. An **unknown config field name now surfaces in `errors`** (and produces no bytes) instead of silently encoding an empty submessage the device would ack while changing nothing — so a typo or a stale field name fails loudly. The `application` / `lorawan` field maps cover every LoRaWAN-readable config field, including `battery_level`, `vendor_reset_allow`, `radio_mode`, `lrw_link_check_interval` and `lrw_link_check_fail_rejoin` (the `lorawan` group drops the `lrw_` prefix, so e.g. `lrw_link_check_interval` is authored as `link_check_interval`), and `get_param.page` is honoured for paged reads. Completed as part of the v1.4.0 validation sweep.
...
- **`app_report`** owns *when* to measure & send — the `interval_report` cadence timer, the lazy sample trigger (`interval_sample == 0`) and the history capture — and runs on its **own work queue**, so the sensor read and the history flash write no longer execute on the LoRaWAN TX work queue (or the system work queue that also drives the MAC). Each cycle it **samples and captures history first, then** applies the link gate and (only if joined) calls `app_lrw_send_telemetry()` — so store-and-forward records accumulate even on a device that has not joined, and replay after join drains them (offline capture, HW-verified).
- **History capture happens only on the fixed cadence.** A report cycle can also be run *off-cadence* by an ad-hoc trigger — an alarm uplink, a `force_send`, the `sample` command, or the link-ready kick after a join. Those runs sample and send fresh telemetry but **do not append a history record and do not re-arm the cadence timer**, so history records stay spaced at exactly `interval_report`. This matters because a record carries no per-record timestamp on the wire — the consumer reconstructs `time(j) = t0 + j*interval_s` — so an off-cadence append (and the shifted timer it would leave behind) would skew that record and every one after it. Fixed as part of the v1.4.0 validation sweep.
