...
- **Atomic ingest (#H-11)** — a config record is applied **all-or-nothing**: the ingest snapshots the running config, applies each present group, and on the **first invalid field rolls the whole record back** (nothing is applied) and logs the fault. Previously the config-record path was best-effort — it applied the valid fields and silently ignored the invalid one, so a record with one bad field left the device **half-provisioned while looking like a success**. (The `set_param` command path was already atomic; this brings the bare config-record path in line — both now use the same validating `apply_*` functions.)
