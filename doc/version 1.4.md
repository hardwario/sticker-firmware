...
- **Audit fixes — telemetry cadence & framing (#267):**
  - **Report jitter sign fix** — the ±jitter added to `interval_report` used a **signed** modulo (`(int32_t)sys_rand32_get() % …`), which is negative ~half the time, so reports fired **early** and the real uplink rate ran above the configured interval. It is now an unsigned `0..interval/10` jitter (always ≥ 0, spreading later only), with a guard against `interval_report < 10` (modulo-by-zero).
  - **Telemetry frame buffer sizing** — the fPort-2 compose buffer was a hardcoded 64 B, so `app_compose()` (which caps a frame at `min(buffer, live DR budget)`) fragmented a snapshot into **more uplinks than the data rate required** on higher DRs (EU868 DR4–6 carry up to 242 B) — extra TX + RX windows + duty cycle + battery. The buffer is now sized to the max LoRaWAN application payload (242 B), so each frame fills the actual DR budget.
