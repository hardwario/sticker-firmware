...
- **Code de-duplication (#220)** — follow-on to the PSA drop, reclaiming flash by collapsing repeated boilerplate without behaviour change (release figures, measured against the post-#226 base 161 256 B / 98.42 %):
  ...
  - **GPIO edge/count helper** — `app_hall` and `app_input` share `app_gpio_count_apply()` for the per-channel edge-detect + counter + transition-log step (each keeps its own sampling: hall's pull-up toggle, input's plain read, and its own public API). **−176 B.**
  ...
  - Net release: **161 256 → 160 324 B (97.85 %)**, headroom 2 584 → 3 516 B.
