...
It clears (`event:"deactivate"`) once the supply recovers past a **fixed 0.3 V** hysteresis band (with the 2.4 V default: alarm below 2.4 V, clears above 2.7 V) — the hysteresis is a compile-time constant, not configurable. The event carries the measured voltage in `value` and uses the reserved slot `254` (0xFE; the no-data watchdog uses `255`). Being an alarm, it also **blinks the red LED**. The threshold is configurable (`config battery-level <mV>`, or `battery_level` over SetParam in the `application` group, proto field 6) because Li cells discharge non-linearly — a fixed empty point would mis-warn across chemistries.
