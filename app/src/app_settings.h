 ...
 *                    command / `settings reset`) — this is the only reset
 *                    reachable remotely via device_reset, so no command can
 *                    ever un-provision a field device down to this tier.
 *   factory_reset  - keeps identity only; drops the LoRaWAN session/keys —
 *                    the device must re-join after this.
 ...
 *                    partition is raw-erased. Shell-only for now
 *                    (`settings vendor-reset`) — the NFC hio.stck:rst
 *                    vendor-token channel that reaches it remotely lands in a
 *                    later stage. */
 ...
 * encrypted NFC channel (secret_key_is_set() gate in app_nfc.c), with no way
 * back in short of a J-Link. So the caller MUST supply the replacement key in
 * the same call; `new_secret_key` (16 bytes) is rejected (-EACCES) if NULL or
 * all-zero, same as the `vendor_reset_allow` config gate below. Returns
