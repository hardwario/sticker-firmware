...
int main(void)
{
	...
	for (;;) {
		...
		if (lrw_state == APP_LRW_STATE_JOINING || lrw_state == APP_LRW_STATE_RECONNECT) {
			...
			/* Radio disabled by lrw-mode (#271): same single heartbeat blink as a
			 * healthy device but ORANGE instead of green, so an operator can tell
			 * "radio is off (by config)" apart from a joined device at a glance.
			 * DISABLED now means lrw-mode OFF/P2P — the old blank-DevEUI radio-
			 * silent guard is gone (#98/#175 superseded by #271). */
			struct app_led_blink_req req = {.color = APP_LED_CHANNEL_Y,
							.duration = 5,
							.space = 0,
							.repetitions = 1};
