...
/*
 * Wire frame (raw LoRa has no addressing/MIC/encryption of its own, #119):
 *
 *   [ net_id(4 BE) | dev_addr(2 BE) | frame_type(1) | counter(4 BE) ]  11 B header
 *   [ AES-CCM ciphertext (= plaintext length) ]
 *   [ AES-CCM tag (4 B) ]
 *
 * The header is cleartext (the receiver filters foreign traffic by net_id and
...
/* Fixed in v1 (kept out of config to fit the dual-stack flash budget): the
 * common P2P defaults of 125 kHz bandwidth and 4/5 coding rate. The receiver
 * must match these. Frequency / SF / TX power stay configurable. */
			...
			uint8_t dir)
{
	...
	sys_put_be16((uint16_t)g_app_config.p2p_device_addr, &nonce[4]);
	nonce[6] = frame_type;
	nonce[7] = dir;
}
...
static int tx_frame(uint8_t frame_type, const uint8_t *body, size_t body_len)
{
	...
	sys_put_be32(g_app_config.p2p_network_id, &frame[0]);
	sys_put_be16((uint16_t)g_app_config.p2p_device_addr, &frame[4]);
	frame[6] = frame_type;
	...
	int ret = app_ccm_encrypt(g_app_config.p2p_key, P2P_KEY_LEN, nonce, P2P_NONCE_LEN,
				  /* AAD */ frame, P2P_HDR_LEN, body, body_len, &frame[P2P_HDR_LEN],
				  &frame[P2P_HDR_LEN + body_len], P2P_TAG_LEN);
	if (ret) {
		LOG_ERR_CALL_FAILED_INT("app_ccm_encrypt", ret);
		return ret;
	}
...
static void send_work_handler(struct k_work *work)
{
	...
	do {
		int ret = app_compose_ex(buf, sizeof(buf), &len, &more, P2P_MAX_BODY);
		...
		if (ret) {
			LOG_ERR_CALL_FAILED_INT("app_compose_ex", ret);
			return;
		}
		if (len == 0) {
...
static void rx_work_handler(struct k_work *work)
{
	...
	while (k_msgq_get(&m_rx_msgq, &msg, K_NO_WAIT) == 0) {
		...
		if (net_id != g_app_config.p2p_network_id) {
		...
		int ret = app_ccm_decrypt(g_app_config.p2p_key, P2P_KEY_LEN, nonce, P2P_NONCE_LEN,
					  /* AAD */ msg.buf, P2P_HDR_LEN, &msg.buf[P2P_HDR_LEN],
					  ct_len, &msg.buf[P2P_HDR_LEN + ct_len], P2P_TAG_LEN, pt);
		if (ret) {
		...
		/* Only inbound COMMAND frames are fed to the command dispatcher; the
		 * uplink types (telemetry/alarm/response) are decoded off-device. */
		if (frame_type == APP_P2P_FRAME_COMMAND) {
			uint8_t resp[P2P_TX_BUF_SIZE];
			size_t resp_len = 0;
			enum app_cmd_action action = APP_CMD_ACTION_NONE;

			ret = app_cmd_handle(APP_CMD_TRANSPORT_LRW, pt, ct_len, resp, sizeof(resp),
					     &resp_len, &action);
			if (ret) {
...
int app_p2p_listen(bool enable)
{
	...
	if (enable) {
		...
		m_listening = true;
		LOG_INF("P2P listen mode ON (RX)");
	} else {
		(void)lora_recv_async(m_lora_dev, NULL, NULL); /* cancel */
		m_listening = false;
		(void)radio_configure(true);
		LOG_INF("P2P listen mode OFF (TX)");
	}
	return 0;
}
...
/* ======================================================================== */
/* Watchdog liveness heartbeat (#182)                                       */
/* ======================================================================== */
...
int app_p2p_init(void)
{
	...
	if (ret && ret != -EEXIST) {
		LOG_ERR_CALL_FAILED_INT("settings_register(p2pfc)", ret);
	}
	ret = settings_load_subtree(P2P_FCNT_SUBTREE);
	if (ret) {
		LOG_ERR_CALL_FAILED_INT("settings_load_subtree(p2pfc)", ret);
	}
	/* Reserve a fresh counter window up front: any counter below the persisted
	 * high-water may already be on the air, so jump past it before the first TX. */
	fcnt_reserve(m_fcnt + P2P_FCNT_RESERVE);
	...
	for (size_t i = 0; i < P2P_KEY_LEN; i++) {
		if (g_app_config.p2p_key[i] != 0) {
			key_zero = false;
			break;
		}
	}
	if (key_zero) {
		LOG_WRN("P2P key is all-zero — link is effectively unprotected; provision p2p_key");
	}
	...
	LOG_INF("P2P transport ready: %u Hz, SF%d, BW%u, addr %u, net %u",
		g_app_config.p2p_frequency, sf_from_cfg(), P2P_BANDWIDTH_HZ / 1000,
		(uint16_t)g_app_config.p2p_device_addr, g_app_config.p2p_network_id);
	return 0;
}
...
{
	m_started = true;
	/* No join handshake: the link is up immediately, so kick app_report to
	 * begin the report cadence with an immediate uplink (mirrors the LoRaWAN
	 * on-join-success ready edge). */
	if (m_ready_cb) {
		m_ready_cb();
	}
}
...
int app_p2p_queue_response(uint8_t port, const uint8_t *buf, size_t len)
{
	...
	if (len > P2P_TX_BUF_SIZE) {
		LOG_ERR("Response too large: %zu B (max %d)", len, P2P_TX_BUF_SIZE);
		return -EMSGSIZE;
	}

	struct p2p_tx_msg msg;

	msg.type = APP_P2P_FRAME_RESPONSE;
	msg.len = len;
	memcpy(msg.buf, buf, len);

	if (k_msgq_put(&m_tx_msgq, &msg, K_NO_WAIT) != 0) {
		LOG_WRN("TX queue full; dropping response");
		return -ENOMEM;
	}
...
int app_p2p_send_alarm(const uint8_t *buf, size_t len)
{
	...
	if (len > P2P_TX_BUF_SIZE) {
		LOG_ERR("Alarm batch too large: %zu B (max %d)", len, P2P_TX_BUF_SIZE);
		return -EMSGSIZE;
	}

	struct p2p_tx_msg msg;

	msg.type = APP_P2P_FRAME_ALARM;
	msg.len = len;
	memcpy(msg.buf, buf, len);

	if (k_msgq_put(&m_tx_msgq, &msg, K_NO_WAIT) != 0) {
		LOG_WRN("TX queue full; dropping alarm");
		return -ENOMEM;
	}
	k_work_submit_to_queue(&m_work_q, &m_tx_work);
	return 0;
...
{
	/* Nothing periodic to stop here — the report cadence lives in app_report,
	 * which app_power_suspend() stops separately. */
	m_started = false;
}
