 ...
 * capture, then triggers the transport via app_radio_lrw_send_telemetry(). It runs on
 ...
 *
 * Layering: app_report = *when* to measure & send; app_radio_lrw = *how* it reaches the
 ...
 * only reads app_radio_lrw (app_radio_lrw_get_state / app_radio_lrw_is_ready); app_radio_lrw kicks
 ...
 * register the link-ready kick with app_radio_lrw. Does NOT start the cadence — that
 * begins on the first link-ready kick from app_radio_lrw (join success). Returns 0 on
 * success or a negative errno. */
int app_report_init(void);

/* Force an immediate report cycle (`send` shell, alarm uplink). Sample-if-lazy +
 * trigger app_radio_lrw_send_telemetry() (with its fleet pre-send jitter); no history
 ...
 * (app_radio_lrw_send_telemetry_now()): the answer to a host command (force_send,
 ...
 * queued on m_work_q so it can't run app_radio_lrw_send_telemetry() after this
